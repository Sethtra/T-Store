...
- Google OAuth uses a short-lived one-time code exchange so Sanctum tokens are not placed in browser URLs.
...
| `POST` | `/api/auth/google/exchange` | Exchange one-time Google login code for a Sanctum token |
...
- The frontend can be auto-deployed by Vercel from the `main` branch.
- The backend can be deployed from GitHub Actions by SSHing into the EC2 server, pulling `main`, installing Composer dependencies, running migrations, caching Laravel config/routes/views, and restarting queues.
- Keep production `.env` files on the server or hosting provider only.

Example backend deploy steps:
...
- Google OAuth uses a one-time code exchange instead of redirecting tokens through the browser URL.
