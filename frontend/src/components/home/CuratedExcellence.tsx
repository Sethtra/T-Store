    ...
    >
      <div className="sticky top-0 h-screen flex items-center overflow-hidden">
        <motion.div
          ...
          style={{ x: xTransform }}
          className="flex gap-4 sm:gap-8 px-4 sm:px-8 lg:px-32 w-max"
        >
          {/* Intro text for horizontal scroll */}
          <div className="w-[90vw] sm:w-[40vw] shrink-0 flex flex-col justify-center pr-4 sm:pr-12">
          ...
          {[
            ...
            <div
              key={i}
              className="w-[85vw] sm:w-[50vw] md:w-[40vw] h-[50vh] sm:h-[60vh] shrink-0 relative rounded-[2rem] sm:rounded-[3rem] overflow-hidden group"
            >
              <img
                src={item.img}
                alt={item.title}
              ...
              <div className="absolute bottom-0 left-0 p-6 sm:p-10 w-full transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                <p className="text-indigo-400 font-bold uppercase tracking-widest text-xs sm:text-sm mb-2 sm:mb-3">
                  {item.sub}
                </p>
                ...
                <Link to="/products">
                  <button className="opacity-0 group-hover:opacity-100 px-6 sm:px-8 py-3 sm:py-4 bg-white text-black rounded-full font-bold flex items-center gap-2 hover:bg-zinc-200 transition-all duration-500 delay-100 text-sm sm:text-base">
